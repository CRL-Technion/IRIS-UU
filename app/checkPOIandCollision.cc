...
int main(int argc, char **argv)
{
	...
	// fout_result.open(file_to_read + "_resultDetail");

	// if (!fout_result.is_open())
	// {
	// 	std::cerr << file_to_read + "_result"
	// 			  << " cannot be opened!" << std::endl;
	// 	exit(1);
	// }
	// bool isTherePOI = false;
	// for (size_t i = 0; i < graph->NumVertices(); i++)
	// {
	// 	isTherePOI = false;
	// 	// fout_result << " "<< graph->Vertex(i)->vis.Size() ;
	// 	for (size_t j = 0; j < MAX_COVERAGE_SIZE; j++)
	// 	{
	// 		if (graph->Vertex(i)->vis.bitset_[j] > 0.5)
	// 		{
	// 			fout_result << " " << j;
	// 			isTherePOI = true;
	// 		}
